...
class NnValueFunction(object):
    ...
    def init_net(self,shape):
        ...
        h2 = lrelu(ense(h1, 16, 'h2', tf.random_uniform_initializer(-1.0, 1.0)))
...
def main_pendulum(logdir, seed, n_iter, gamma, min_timesteps_per_batch, initial_stepsize, desired_kl, vf_type, vf_params, animate=False):
    ...
    #YOUR_CODE_HERE#
    sy_ob_no = tf.placeholder(shape = [None,ob_dim],name = "pb",dtype = tf.float32)
    ...
    sy_mean_na = dense(sy_h2, ac_dim, "final", weight_init=normc_initializer(0.1))
    ...
    for i in range(n_iter):
        ...
        #collect data after one run of game
        path = {"observation":np.array(obs),"terminated":terminated,
                "reward":np.array(rewards),"action":np.array(acs)}
        paths.append(path)
        timesteps_this_batch += pathlength(path)
