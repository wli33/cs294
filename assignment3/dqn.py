...
def learn(env,
    ...
    act_t = tf.one_hot(action,num_actions,dtype=tf.float32, name="action_one_hot")
    q_act_t = tf.reduce_sum(act_t,current_q_func,axis = 1)

    y = rew_t_ph + gamma * tf.reduce_max(target_q_func, axis=1)
    total_error = tf.square(y-q_act_t)
    ...
    for t in itertools.count():
        ...
        if not model_initialized or random()<epsilon:
        ...
        if (t > learning_starts and
            ...
            #####
            # YOUR CODE HERE
            obs_t_batch,act_batch,rew_batch,next_obs_tp1_batch,done_mask = replay_buffer.sample(batch_size)
