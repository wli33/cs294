...
class NeuralNetworks:
    ...
    def weight_variable(shape,value = 0.1,rand = 'normal'):
        if rand == 'normal':
            initial = tf.trunctated_notmal(shape,stddev = value)
        elif rand == 'uniform':
            initial = tf.random_uniform(shape, minval= -value,maxval = value)
        return tf.Vaiable(initial)

    def bias_variable(shape,valur = 0.01):
    ...
    def _create_state_feature(self,sigma = 0.1):
        ...
        w = NeuralNetworks.weight_variable([self._S_DIM,self._S_FEATURE_DIM],value=sigma)
        b = NeuralNetworks.bias_variable([self._FEATURE_DIM])
